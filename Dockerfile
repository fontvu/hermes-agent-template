...
# ── In-tree patch: CommandCode provider model-catalog fetch ──────────────────
# Upstream's commandcode plugin overrides fetch_models() WITHOUT the base_url
# kwarg that hermes_cli.models.provider_model_ids() now passes (the calling
# convention was standardized to fetch_models(api_key=***, base_url=***,
# timeout=***) for the generic live-catalog path). The call raises
