...
# ── Login rate limiting ─────────────────────────────────────────────────────
# In-memory per-IP lockout: after LOGIN_MAX_ATTEMPTS failed logins within
# LOGIN_LOCKOUT_SECONDS, further attempts from that IP are rejected until
# enough of the window has elapsed. State is process-local — acceptable here
# since Railway runs a single instance of this service — and is pruned lazily
# on each check, so it never grows without bound in practice.
...
def _client_ip(request: Request) -> str:
    """Best-effort client IP, honoring Railway's X-Forwarded-For edge header."""
...
def _is_locked_out(ip: str) -> bool:
    now = time.time()
    attempts = [t for t in _login_failures.get(ip, []) if now - t < LOGIN_LOCKOUT_SECONDS]
...
async def login_post(request: Request) -> Response:
    ...
    if _is_locked_out(ip):
        return RedirectResponse(
            f"/login?returnTo={_url_quote(return_to)}&error=locked", status_code=302
        )
